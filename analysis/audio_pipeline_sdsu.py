...
SDSU_FOLDER = "Participant-1-02232026"
...
def preprocess_and_segment(filepath):
    ...
    # Chop into 1.5s segments using an overlapping sliding window (0.5s hop)
...
def build_dataset():
    """Walk raw_data/Participant-1-02232026/ and build the full feature dataset for binary classification."""
    ...
    folder_path = RAW_DATA / SDSU_FOLDER
    if not folder_path.exists():
        print(f"  ⚠ SDSU folder not found: {folder_path}")
        return [], [], [], [], [], [], [], []
    ...
    print(f"  📂 {SDSU_FOLDER}: {len(pcm_files)} total files")

    for pcm in pcm_files:
        chunks = preprocess_and_segment(str(pcm))
        
        # Determine label based on filename string
        is_idle = "_idle_" in pcm.stem.lower()
        label = 0 if is_idle else 1
        
        # Optional: extract exact food name for metadata (e.g., 'coke', 'cracker', 'idle')
        file_parts = pcm.stem.split('_')
        food_name = file_parts[1] if len(file_parts) > 1 else "unknown"
        ...
        for ci, seg in enumerate(chunks):
            ...
            labels.append(label)
            meta_info.append((SDSU_FOLDER, pcm.name, ci, food_name))
...
def main():
    ...
    if results_rows:
        ...
        out_csv = SCRIPT_DIR / "pipeline_sdsu_results.csv"
